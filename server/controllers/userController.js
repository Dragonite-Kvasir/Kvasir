...
userController.getInfo = async (req, res, next) => {
  try {
    ...
    const q1 = `SELECT display_name FROM users WHERE _id=${res.locals.id}`;
    let d1 = await db.query(q1);
    if(d1.rows[0].display_name) res.locals.displayName = d1.rows[0].display_name;

    const q2 = `SELECT * FROM user_interests i LEFT OUTER JOIN interests ON i.interest_id = interests._id WHERE i.user_id=${res.locals.id}`;
    let d2 = await db.query(q2);
    if(d2.rows[0]) res.locals.interests = d2.rows.map(e => e.name);
    else res.locals.interests = [];

    const q3 = `SELECT * FROM user_teach_languages LEFT OUTER JOIN languages ON user_teach_languages.language_id = languages._id WHERE user_teach_languages.user_id = ${res.locals.id}`;
    let d3 = await db.query(q3);
    if(d3.rows[0]) res.locals.canTeach = d3.rows.map(e => e.name);
    else res.locals.canTeach = [];

    const q4 = `SELECT * FROM user_learn_languages LEFT OUTER JOIN languages ON user_learn_languages.language_id = languages._id WHERE user_learn_languages.user_id = ${res.locals.id}`;
    let d4 = await db.query(q4);
    if(d4.rows[0]) res.locals.canLearn = d4.rows.map(e => e.name);
    else res.locals.canLearn = [];
